...
function LoginScreen() {
  ...
  async function handleGoogleLogin() {
    ...
    if (error) { toast.error("Hata: " + error.message); setLoading(false); }
  }
      ...
      <Card className="w-full max-w-sm">
        <CardContent className="pt-8 pb-8 space-y-6">
          ...
          <div className="space-y-3">
            <Button className="w-full bg-white hover:bg-gray-50 text-gray-800 border border-gray-300" onClick={handleGoogleLogin} disabled={loading} size="lg">
              {loading ? (
                <Loader2 className="size-4 mr-2 animate-spin text-gray-600" />
              ) : (
            ...
            <p className="text-xs text-center text-muted-foreground">
              Google hesabınla güvenli ve hızlı giriş yap.
            </p>
          </div>
        </CardContent>
      </Card>
